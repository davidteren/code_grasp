...
class CodeDB:
    ...
    def add_embeddings(self, embeddings, file_paths):
        ...
        if self.dim is None:
            ...
            print(
                f"Warning: Embedding dimension mismatch. Expected {self.dim}, got {embedding_dim}."
            )
    ...
    def search(self, query_embedding, k=5):
        ...
        if query_embedding.shape[1] != self.dim:
            print(
                f"Error: Query embedding dimension {query_embedding.shape[1]} doesn't match index dimension {self.dim}"
            )
            return []
        ...
        for idx, score in zip(indices[0], scores[0]):
            if idx != -1:  # Valid index
                self.cursor.execute(
                    "SELECT file_path, file_language FROM code WHERE embedding_id = ?", (int(idx),)
                )
                result = self.cursor.fetchone()
                if result:
