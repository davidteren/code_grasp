        ...
        if len(file_paths) == 0:
            ...
            for path in file_paths:
                ...
                language_map = {
                    ...
                    ".ts": "Typescript",
                    ".jsx": "Javascript",
                    ".tsx": "Typescript",  # Add React file types
                    ".html": "HTML",
                    ".css": "CSS",  # Add web file types
                }
...
def ask_dir(directory, question, lightweight, batch_size):
    ...
    try:
        ...
        if query_embedding.shape[1] != dim:
            click.echo(
                f"Warning: Query embedding dimension ({query_embedding.shape[1]}) doesn't match file embeddings dimension ({dim})."
            )
...
def info(lightweight=False):
    ...
    try:
        ...
        if count == 0:
            click.echo(
                "No files have been added yet. Use 'code_grasp add <directory>' to add files."
            )
            db.close()
            return
        ...
        db.cursor.execute(
            "SELECT file_language, COUNT(*) FROM code GROUP BY file_language ORDER BY COUNT(*) DESC"
        )
        ...
        # Get index size
        index_size_mb = (
            os.path.getsize(db.index_path) / (1024 * 1024) if os.path.exists(db.index_path) else 0
        )
        db_size_mb = (
            os.path.getsize(db.db_path) / (1024 * 1024) if os.path.exists(db.db_path) else 0
        )
